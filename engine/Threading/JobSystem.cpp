...
uint32_t JobSystem::submitJob(JobType type, const JobPayload& payload, std::function<JobResult()> work) {
    ...
    while (!m_shutdown.load()) {
        ...
        // Wait for work or shutdown
        {
            ...
            m_workCondition.wait(lock, [this] { 
                return m_shutdown.load() || !m_workQueue.empty() || m_paused.load(); 
            });
...
namespace Jobs {
    uint32_t submitChunkMesh(uint32_t chunkID, uint32_t islandID, const void* voxelData, size_t dataSize) {
        ...
        return g_jobSystem.submitJob(JobType::CHUNK_MESHING, payload, [=]() -> JobResult {
            JobResult result;
    ...
    uint32_t submitPhysicsCook(uint32_t chunkID, const void* meshData, size_t meshSize) {
        ...
        return g_jobSystem.submitJob(JobType::PHYSICS_COOKING, payload, [=]() -> JobResult {
            JobResult result;
            result.chunkID = chunkID;
            result.success = true;
    ...
    uint32_t submitLODGeneration(uint32_t chunkID, int lodLevel, const void* sourceData) {
        ...
        return g_jobSystem.submitJob(JobType::LOD_GENERATION, payload, [=]() -> JobResult {
            JobResult result;
            result.chunkID = chunkID;
            result.success = true;
    ...
    uint32_t submitWorldGeneration(uint32_t seed, int chunkX, int chunkY, int chunkZ) {
        ...
        return g_jobSystem.submitJob(JobType::WORLD_GENERATION, payload, [=]() -> JobResult {
            JobResult result;
