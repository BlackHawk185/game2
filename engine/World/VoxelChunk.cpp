...
void VoxelChunk::setRawVoxelData(const uint8_t* data, uint32_t size)
{
    if (!data || size != VOLUME)
    {
        std::cerr << "Invalid voxel data size. Expected: " << VOLUME << ", Got: " << size
                  << std::endl;
        return;
    }
...
void VoxelChunk::generateFloatingIsland(int seed)
{
    ...
    if (g_jobSystem.isInitialized())
    {
        ...
        for (int slice = 0; slice < numSlices; slice++)
        {
            ...
            JobPayload payload;
            // Cast chunk pointer to uint32_t (truncation acceptable for demo IDs)
            payload.chunkID = static_cast<uint32_t>(
                reinterpret_cast<uintptr_t>(this));  // Use chunk pointer as ID
    ...
    for (int x = 0; x < SIZE; x++)
    {
        for (int y = 0; y < SIZE; y++)
        {
            for (int z = 0; z < SIZE; z++)
            {
                ...
                {
                    glPushMatrix();
                    glTranslatef(static_cast<float>(x), static_cast<float>(y),
                                 static_cast<float>(z));
        ...
        for (int y = 0; y < SIZE; y += step)
        {
            for (int z = 0; z < SIZE; z += step)
            {
                if (this->getVoxel(x, y, z) != 0)
                {
                    ...
                    {
                        // All faces
                        if (this->shouldRenderFace(x, y, z, 0))
                        {  // Front
                            glNormal3f(0, 0, 1);
                            glVertex3f(0.0f, 0.0f, 1.0f);
                            glVertex3f(static_cast<float>(step), 0.0f, 1.0f);
                            glVertex3f(static_cast<float>(step), static_cast<float>(step), 1.0f);
                            glVertex3f(0.0f, static_cast<float>(step), 1.0f);
                        }
                        if (this->shouldRenderFace(x, y, z, 2))
                        {  // Top
                            glNormal3f(0, 1, 0);
                            glVertex3f(0.0f, static_cast<float>(step), 0.0f);
                            glVertex3f(0.0f, static_cast<float>(step), static_cast<float>(step));
                            glVertex3f(static_cast<float>(step), static_cast<float>(step),
                                       static_cast<float>(step));
                            glVertex3f(static_cast<float>(step), static_cast<float>(step), 0.0f);
                        }
                    }
                    else
                    {
