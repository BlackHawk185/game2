...
RayHit VoxelRaycaster::raycast(const Vec3& origin, const Vec3& direction, float maxDistance, IslandChunkSystem* islandSystem) {
    return performDDA(origin, direction, maxDistance, islandSystem);
}

RayHit VoxelRaycaster::raycast(const Vec3& origin, const Vec3& direction, float maxDistance, VoxelChunk* voxelChunk) {
...
RayHit VoxelRaycaster::performDDA(const Vec3& rayStart, const Vec3& rayDirection, float maxDistance, IslandChunkSystem* islandSystem) {
    RayHit result;
    ...
    for (VoxelChunk* chunk : islandChunks) {
        ...
        if (!rayIntersectsAABB(localRayStart, rayDirection, Vec3(0,0,0), Vec3(32,32,32), maxDistance)) {
...
RayHit VoxelRaycaster::performDDA(const Vec3& rayStart, const Vec3& rayDirection, float maxDistance, VoxelChunk* voxelChunk) {
    RayHit result;
    
    // std::cout << "🔍 Starting DDA raycast from (" << rayStart.x << "," << rayStart.y << "," << rayStart.z << ")" << std::endl;
    ...
    while (distance < maxDistance) {
        ...
        if (++steps % 50 == 0) {  // Reduce debug frequency
            std::cout << "🔍 Step " << steps << ": Testing world(" << mapX << "," << mapY << "," << mapZ << ")" 
        ...
        if (chunkX >= 0 && chunkX < 32 && chunkY >= 0 && chunkY < 32 && chunkZ >= 0 && chunkZ < 32) {
            ...
            if (steps % 50 == 0) {  // Reduce debug spam
            ...
            if (voxel > 0) {
                // Hit a solid block!
                result.hit = true;
                result.localBlockPos = Vec3(mapX, mapY, mapZ);  // For compatibility, use localBlockPos
                result.chunkX = mapX;
                result.chunkY = mapY;
                result.chunkZ = mapZ;
                if (side == 0) {
                    result.normal = Vec3(-stepX, 0, 0);
                } else if (side == 1) {
                    result.normal = Vec3(0, -stepY, 0);
                } else {
                    result.normal = Vec3(0, 0, -stepZ);
                }
...
RayHit VoxelRaycaster::performLocalDDA(const Vec3& localRayStart, const Vec3& rayDirection, float maxDistance, VoxelChunk* chunk) {
    RayHit result;
    ...
    while (distance < maxDistance) {
        ...
        if (mapX >= 0 && mapX < 32 && mapY >= 0 && mapY < 32 && mapZ >= 0 && mapZ < 32) {
            ...
            if (voxel > 0) {
                // Hit a solid block!
                result.hit = true;
                result.localBlockPos = Vec3(mapX, mapY, mapZ);  // LOCAL coordinates
                result.chunkX = mapX;
                result.chunkY = mapY;
                result.chunkZ = mapZ;
...
bool VoxelRaycaster::rayIntersectsAABB(const Vec3& rayStart, const Vec3& rayDir, const Vec3& aabbMin, const Vec3& aabbMax, float maxDist) {
    // Simple AABB ray intersection test
    Vec3 invDir(1.0f / rayDir.x, 1.0f / rayDir.y, 1.0f / rayDir.z);
    
    Vec3 t1((aabbMin.x - rayStart.x) * invDir.x, (aabbMin.y - rayStart.y) * invDir.y, (aabbMin.z - rayStart.z) * invDir.z);
    Vec3 t2((aabbMax.x - rayStart.x) * invDir.x, (aabbMax.y - rayStart.y) * invDir.y, (aabbMax.z - rayStart.z) * invDir.z);
