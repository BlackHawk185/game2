...
void Player::updatePhysics(float deltaTime) {
    ...
    if (g_timeManager) {
        float timeBubbleEffect = g_timeManager->getTimeBubbleEffect(position.x, position.y, position.z);
...
void Player::checkGroundCollision() {
    ...
    int checkX = (int)floor(localPos.x);
    int checkY = (int)floor(localPos.y - 0.1f); // Check slightly below feet
    int checkZ = (int)floor(localPos.z);
    
    // Get voxel at feet position
    uint8_t voxelBelow = g_islandSystem.getVoxelFromIsland(mainIslandID, Vec3(checkX, checkY, checkZ));
    
    bool wasInContactWithIsland = false;
    
    if (voxelBelow != 0) { // Solid voxel found
        onGround = true;
    ...
    if (!onGround && velocity.y < 0) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                int testX = checkX + dx;
                int testZ = checkZ + dz;
                uint8_t testVoxel = g_islandSystem.getVoxelFromIsland(mainIslandID, Vec3(testX, checkY, testZ));
...
void Player::applyFriction(float deltaTime) {
    if (onGround) {
        ...
        float frictionFactor = pow(groundFriction, deltaTime * 60.0f); // Normalize to 60 FPS
        velocity.x *= frictionFactor;
        velocity.z *= frictionFactor;
    } else {
        // Frame-rate independent air resistance  
        float airFrictionFactor = pow(airFriction, deltaTime * 60.0f); // Normalize to 60 FPS
