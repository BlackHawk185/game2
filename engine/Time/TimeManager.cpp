...
void TimeManager::updateTransitions(float realDeltaTime)
{
    for (auto it = m_transitions.begin(); it != m_transitions.end();)
    {
        ...
        float currentScale =
            transition.startScale + (transition.targetScale - transition.startScale) * t;
...
void TimeManager::createTimeBubble(const std::string& name, float x, float y, float z, float radius,
                                   float timeScale, float duration)
{
    TimeBubble bubble;
    bubble.x = x;
    bubble.y = y;
...
void TimeManager::debugPrintTimeInfo() const
{
    ...
    for (const auto& [scale, timeInfo] : m_timeScales)
    {
        std::cout << scaleNames[i++] << ": scale=" << timeInfo.timeScale
                  ...
                  << ", dt=" << timeInfo.deltaTime << "s" << (timeInfo.isPaused ? " [PAUSED]" : "")
                  << std::endl;
    }
    ...
    if (!m_transitions.empty())
    {
        ...
        for (const auto& [scale, transition] : m_transitions)
        {
            std::cout << "  Scale " << (int) scale << ": " << transition.startScale << " -> "
                      << transition.targetScale << " (" << transition.elapsed << "/"
                      << transition.duration << "s)" << std::endl;
        }
    }
}
