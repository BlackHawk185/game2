...
{
    if (!m_islandSystem) return false;

    std::cout << "[PHYSICS] Checking player collision at (" << playerPos.x << ", " << playerPos.y << ", " << playerPos.z << ") (radius: " << playerRadius << ")" << std::endl;
    ...
    for (const auto& islandPair : islands)
    {
        ...
        if (!island || !island->mainChunk) continue;

        // Convert player position to island-local coordinates
        Vec3 localPlayerPos = playerPos - island->physicsCenter;

        std::cout << "[PHYSICS] Checking island " << islandPair.first << " at (" << island->physicsCenter.x << ", " << island->physicsCenter.y << ", " << island->physicsCenter.z << ") (local pos: (" << localPlayerPos.x << ", " << localPlayerPos.y << ", " << localPlayerPos.z << "))" << std::endl;

        // Check collision with this island's chunk
        Vec3 collisionNormal;
        if (checkChunkCollision(island->mainChunk.get(), localPlayerPos, island->physicsCenter, collisionNormal, playerRadius))
        {
            outNormal = collisionNormal;
            std::cout << "[PHYSICS] COLLISION DETECTED with island " << islandPair.first
                      << " (normal: " << collisionNormal.x << ", " << collisionNormal.y << ", " << collisionNormal.z << ")" << std::endl;
            return true;
        }
    }
...
bool PhysicsSystem::checkRayCollision(const Vec3& rayOrigin, const Vec3& rayDirection, float maxDistance, Vec3& hitPoint, Vec3& hitNormal)
{
    if (!m_islandSystem) return false;
    ...
    for (const auto& islandPair : islands)
    {
        ...
        if (island->mainChunk->checkRayCollision(localRayOrigin, rayDirection, maxDistance, localHitPoint, localHitNormal))
        {
...
bool PhysicsSystem::checkChunkCollision(const VoxelChunk* chunk, const Vec3& playerPos, const Vec3& chunkWorldPos, Vec3& outNormal, float playerRadius)
{
    std::cout << "[PHYSICS] Checking chunk collision at world pos (" << chunkWorldPos.x << ", " << chunkWorldPos.y << ", " << chunkWorldPos.z << ") (local player pos: (" << playerPos.x << ", " << playerPos.y << ", " << playerPos.z << "))" << std::endl;

    // Check if collision mesh needs updating
    const CollisionMesh& collisionMesh = chunk->getCollisionMesh();
    std::cout << "[PHYSICS] Collision mesh has " << collisionMesh.faces.size() << " faces (needsUpdate: "
              << (collisionMesh.needsUpdate ? "true" : "false") << ")" << std::endl;
    ...
    if (collisionMesh.needsUpdate)
    {
        ...
        std::cout << "[PHYSICS] Collision mesh built with " << chunk->getCollisionMesh().faces.size() << " faces" << std::endl;
    }
